 ...
 * nature — CHIP never asks for a signature to browse.
 */
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <ChipMark size={20} className="text-ink" />
              <h2 className="display-wide text-sm tracking-[0.18em]">Connect</h2>
            </div>
            <p className="mt-2 text-sm text-silver">Robinhood Chain · 4663. Connecting only reads your balances; CHIP never asks you to sign anything from this site.</p>
              ...
              {list.map((c) => (
                <li key={c.uid}>
                  <button
                    ...
                    disabled={isPending}
                    onClick={() => pick(c.uid)}
                    className="flex w-full items-center gap-3 rounded-md border border-graphite bg-white/[0.02] px-4 py-3 text-left transition hover:border-energy/60 hover:bg-white/[0.04] disabled:opacity-60"
                  >
                    {c.icon ? (
                      ...
                      <span className="flex h-7 w-7 items-center justify-center rounded-md bg-graphite text-silver">
                        <ChipMark size={14} energy={false} />
                      </span>
                    )}
                    <span className="flex-1 text-sm font-medium">{c.name}</span>
                    {pendingId === c.uid ? <span className="label animate-power-pulse">Waiting…</span> : <span className="label">{c.type === "walletConnect" ? "QR" : "Browser"}</span>}
                  </button>
                </li>
              ))}
            ...
            {failed || error ? (
              <p className="mt-3 text-xs text-energy-2" role="alert">
